...
class KDTree
{
  ...
  public:
    ...
    void swap(Point<Dim> & point_1, Point<Dim> & point_2);
    void buildTree( vector<Point<Dim>>& newPoints, KDTreeNode*& curr, int dim);
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, int dim, KDTreeNode* node) const;
